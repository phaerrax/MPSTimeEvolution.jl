...
function siam_tdvp2(; dt, tmax, freqs, couplings, check_sites, init, cutoff)
    ...
    state_0 = MPS(sites, init)
    maxdim = maximum(maxlinkdims(state_0))  # Allow the MPS to grow as much as it can.
...
function siam_tdvp2_ic(; dt, tmax, freqs, couplings, check_sites, init, cutoff)
    N = length(freqs)
    sites = siteinds("Fermion", N)

    state_0 = InverseCanonicalMPS(sites, init)
    maxdim = maximum(maxlinkdims(state_0))  # Allow the MPS to grow as much as it can.
