...
function MPSTimeEvolution.tdvp2_parallel!(
    ...
    if isroot
        ...
        ranks_handle = MPSTimeEvolution.writeheaders_ranks(ranks_file, length(ψ))
    ...
    for s in 1:nsteps
        ψ = begin  # Maybe this could be a function?
            ...
            sts = gather(parent(st), rootrank, comm)
            bts = gather(parent(bt), rootrank, comm)
