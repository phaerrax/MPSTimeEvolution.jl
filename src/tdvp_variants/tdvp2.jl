...
function tdvp2!(ψ::MPS, H::MPO, timestep, endtime; kwargs...)
    ...
    for j in reverse(eachindex(ψ)[1:(end - 1)])
        orthogonalize!(ψ, j)
        apply!(cb, ψ, TDVP2(); t=current_time, site=j, sweepend=true, sweepdir="left")
    end
    compute_norm!(cb, ψ, TDVP2(); current_time=current_time)
...
function tdvp2!(solver, ψ::InverseCanonicalMPS, PH, dt, tmax; kwargs...)
    ...
    # Measure everything once in the initial state.
    current_time = 0.0
    apply!(cb, ψ, TDVP2(); current_time=current_time)
    compute_norm!(cb, ψ, TDVP2(); current_time=current_time)
